...
# MDLBSG v0.1 license-preserving Hugging Face corpus benchmark runner.
...
SOURCE_DIR="${1:-$HOME/Downloads/MDLBSG_HF_LICENSED_CORPORA_V1/corpora}"
ENWIK9_PATH="${ENWIK9_PATH:-$HOME/Downloads/enwik9}"
...
RUN_NAME="MDLBSG_HF_LICENSED_BENCHMARK_EVIDENCE_${UTC_STAMP}"
...
for name in corpus_hf_wikitext corpus_hf_gsm8k corpus_hf_dolly corpus_hf_code_cc0; do
  [[ -d "$SOURCE_DIR/$name" ]] \
    || fail "Required licensed corpus folder is missing: $SOURCE_DIR/$name"
  [[ -f "$SOURCE_DIR/$name/SOURCE.json" ]] \
    || fail "$name is missing SOURCE.json"
  [[ -f "$SOURCE_DIR/$name/DATASET_CARD.md" ]] \
    || fail "$name is missing DATASET_CARD.md"
  [[ -d "$SOURCE_DIR/$name/LICENSES" ]] \
    || fail "$name is missing LICENSES/"
  [[ -f "$SOURCE_DIR/$name/SHA256SUMS.txt" ]] \
    || fail "$name is missing SHA256SUMS.txt"
  (
    cd "$SOURCE_DIR/$name"
    /usr/bin/shasum -a 256 -c SHA256SUMS.txt >/dev/null
  ) || fail "$name internal checksum manifest failed."
done

[[ -f "$ENWIK9_PATH" ]] \
  || fail "enwik9 must be a regular file: $ENWIK9_PATH"

ENWIK9_BYTES="$(/usr/bin/stat -f%z "$ENWIK9_PATH")"
...
cp "$0" "$EVIDENCE/RUN_MDLBSG_V0_1_HF_LICENSED_CORPORA_BENCHMARK.command"
chmod 755 "$EVIDENCE/RUN_MDLBSG_V0_1_HF_LICENSED_CORPORA_BENCHMARK.command"
SCRIPT_SHA="$(/usr/bin/shasum -a 256 "$EVIDENCE/RUN_MDLBSG_V0_1_HF_LICENSED_CORPORA_BENCHMARK.command" | /usr/bin/awk '{print $1}')"
...
lines = []
lines.append("# MDLBSG v0.1 Licensed Hugging Face Corpus Benchmark Results")
lines.append("")
...
{
  echo "$SCRIPT_SHA  RUN_MDLBSG_V0_1_HF_LICENSED_CORPORA_BENCHMARK.command"
  for path in \
...
mkdir -p "$EVIDENCE/corpus_source_metadata"
for name in corpus_hf_wikitext corpus_hf_gsm8k corpus_hf_dolly corpus_hf_code_cc0; do
  mkdir -p "$EVIDENCE/corpus_source_metadata/$name"
  cp "$SOURCE_DIR/$name/SOURCE.json" \
     "$EVIDENCE/corpus_source_metadata/$name/SOURCE.json"
  cp "$SOURCE_DIR/$name/DATASET_CARD.md" \
     "$EVIDENCE/corpus_source_metadata/$name/DATASET_CARD.md"
  cp -R "$SOURCE_DIR/$name/LICENSES" \
     "$EVIDENCE/corpus_source_metadata/$name/LICENSES"
done
...
for path in root.rglob("*"):
    if not path.is_file() or path.name == "RUN_MDLBSG_V0_1_HF_LICENSED_CORPORA_BENCHMARK.command":
        continue
