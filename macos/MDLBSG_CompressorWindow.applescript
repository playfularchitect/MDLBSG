...
on closeMenuHelper()
	...
	set menuExecutablePattern to "^" & homePath & "[.]mdlbsg/MDLBSG Menu[.]app/Contents/MacOS/applet( |$)"
	set oldScriptPattern to "^/usr/bin/osascript .*[/]launcher_dialog[.]applescript"
	try
		do shell script "/usr/bin/pkill -TERM -f " & quoted form of menuExecutablePattern & " >/dev/null 2>&1 || true; /usr/bin/pkill -TERM -f " & quoted form of oldScriptPattern & " >/dev/null 2>&1 || true; /bin/sleep 0.10; /usr/bin/pkill -KILL -f " & quoted form of menuExecutablePattern & " >/dev/null 2>&1 || true"
	end try
end closeMenuHelper
...
on launchMenuHelper()
	my closeMenuHelper()
	set homePath to POSIX path of (path to home folder)
	set helperApp to homePath & ".mdlbsg/MDLBSG Menu.app"
	try
		do shell script "test -d " & quoted form of helperApp & " && test -x " & quoted form of (helperApp & "/Contents/MacOS/applet")
		do shell script "/usr/bin/open -n " & quoted form of helperApp
...
on run
	my logLaunch("run-handler MDLBSG Compressor -> native menu app")
	my launchMenuHelper()
end run

on reopen
	my logLaunch("reopen-handler MDLBSG Compressor -> native menu app")
	my launchMenuHelper()
end reopen
...
on dispatchFreshWorker(thePaths)
	-- App 88 does not start a compressor directly. It submits the
	-- dropped paths to one shared queue. The queue worker owns the
	-- only progress window and processes every queued item in order.
	if not my resolveDest() then return false

	set homePath to POSIX path of (path to home folder)
	set submitterPath to homePath & ".mdlbsg/bin/mdlbsg_queue_submit.py"

	do shell script "test -x " & quoted form of submitterPath

	set submitCommand to "/usr/bin/python3 " & quoted form of submitterPath & " --dest " & quoted form of my sessionDest & " --"

	repeat with currentPath in thePaths
		set submitCommand to submitCommand & " " & quoted form of (currentPath as text)
	end repeat

	set submitResult to do shell script submitCommand

	my logLaunch("app88 queue submission (" & (count of thePaths) & " items): " & submitResult)

	return true
end dispatchFreshWorker

on open theFiles
	my logLaunch("app88 queue open-handler (" & (count of theFiles) & " items)")
	my closeMenuHelper()
